...
BOP_PICKLE_PATH=${SHARED_FOLDER}/${DATASET}/output/${DATA_SPLIT}-${SUB_DATASET}/obj${OBJ_ID}/${DATASET}-${DATA_SPLIT}-obj${OBJ_ID}.p
if test -f "$BOP_PICKLE_PATH"; then
	...
	${PYTORCH_DOCKER} python multi-pose/utils/bop2pickle.py --datasets_path /shared-folder/ --obj_ids ${OBJ_ID} --dataset_split ${DATA_SPLIT} --dataset ${DATASET} > log.out
    else
	${PYTORCH_DOCKER} python multi-pose/utils/bop2pickle.py --datasets_path /shared-folder/ --obj_ids ${OBJ_ID} --dataset_split ${DATA_SPLIT} --dataset ${DATASET} > log.out
    fi
    wait
...
if test "$APPROACH_NAME" = "sundermeyer"; then
    SM_PICKLE_PATH=${SHARED_FOLDER}/${DATASET}/output/${DATA_SPLIT}-${SUB_DATASET}/obj${OBJ_ID}/${APPROACH_NAME}-result-${DATA_SPLIT}-obj${OBJ_ID}.p
    if test -f "$SM_PICKLE_PATH"; then
	echo $(date +%T) " - pickle with results from Sundermeyers approach already exists, skipping..."
    else
	cp ${SHARED_FOLDER}/${DATASET}/output/${DATA_SPLIT}-${SUB_DATASET}/obj${OBJ_ID}/${DATASET}-${DATA_SPLIT}-obj${OBJ_ID}.p ${SM_PICKLE_PATH}
	wait
	${AAE_DOCKER} python -m auto_pose.ae.images2poses ${DATASET}_autoencoder/obj${OBJ_ID} /shared-folder/${DATASET}/output/${DATA_SPLIT}-${SUB_DATASET}/obj${OBJ_ID}/sundermeyer-result-${DATA_SPLIT}-obj${OBJ_ID}.p > log.out
	wait
	echo $(date +%T) "Convert results from Sundermeyer to CSV for BOP toolkit"
	SM_CSV_PATH=${SHARED_FOLDER}/${DATASET}/output/${DATA_SPLIT}-${SUB_DATASET}/obj${OBJ_ID}/${APPROACH_NAME}-obj${OBJ_ID}_${DATASET}-${DATA_SPLIT}-${SUB_DATASET}.csv
	if test -f "$SM_CSV_PATH"; then
	    echo $(date +%T) " - CSV with results from Sundermeyers approach already exists, skipping..."
	else
	    ${PYTORCH_DOCKER} python multi-pose/eval-pickle.py -pi /shared-folder/${DATASET}/output/${DATA_SPLIT}-${SUB_DATASET}/obj${OBJ_ID}/${APPROACH_NAME}-result-${DATA_SPLIT}-obj${OBJ_ID}.p -o /shared-folder/${DATASET}/output/${DATA_SPLIT}-${SUB_DATASET}/obj${OBJ_ID}/sundermeyer-obj${OBJ_ID}_${DATASET}-${DATA_SPLIT}-${SUB_DATASET}.csv > log.out # -op multi-pose/data/${DATASET}-obj${OBJ_ID}/cad/obj_${OBJ_ID}.ply
	    wait
	fi
    fi
else
    OUR_CSV_PATH=${SHARED_FOLDER}/${DATASET}/output/${DATA_SPLIT}-${SUB_DATASET}/obj${OBJ_ID}/${APPROACH_NAME}-obj${OBJ_ID}_${DATASET}-${DATA_SPLIT}-${SUB_DATASET}.csv
...
RESULT_CSV=/shared-folder/${DATASET}/output/${DATA_SPLIT}-${SUB_DATASET}/obj${OBJ_ID}/${APPROACH_NAME}-obj${OBJ_ID}_${DATASET}-${DATA_SPLIT}-${SUB_DATASET}.csv
BOP_EVAL_PATH=/shared-folder/${DATASET}/output/${DATA_SPLIT}-${SUB_DATASET}/obj${OBJ_ID}/eval
LOCAL_BOP_EVAL_PATH=${SHARED_FOLDER}/${DATASET}/output/${DATA_SPLIT}-${SUB_DATASET}/obj${OBJ_ID}/eval/${APPROACH_NAME}-obj${OBJ_ID}_${DATASET}-${DATA_SPLIT}_${SUB_DATASET}
...
${PYTORCH_DOCKER} python bop_toolkit/scripts/show_performance_bop19.py --result_filenames=${RESULT_CSV} --datasets_path /shared-folder/ --eval_path ${BOP_EVAL_PATH} > log.out
