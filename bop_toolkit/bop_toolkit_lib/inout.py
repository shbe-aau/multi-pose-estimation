  ...
  for im_id in scene_camera.keys():
    if 'cam_K' in scene_camera[im_id].keys():
      scene_camera[im_id]['cam_K'] = \
        np.array(scene_camera[im_id]['cam_K'], np.float_).reshape((3, 3))
    if 'cam_R_w2c' in scene_camera[im_id].keys():
      scene_camera[im_id]['cam_R_w2c'] = \
        np.array(scene_camera[im_id]['cam_R_w2c'], np.float_).reshape((3, 3))
    if 'cam_t_w2c' in scene_camera[im_id].keys():
      scene_camera[im_id]['cam_t_w2c'] = \
        np.array(scene_camera[im_id]['cam_t_w2c'], np.float_).reshape((3, 1))
  return scene_camera
      ...
      for line in f:
        ...
        if line_id == 1 and header in line:
          ...
          result = {
            ...
            'score': float(elems[3]),
            'R': np.array(
              list(map(float, elems[4].split())), np.float_).reshape((3, 3)),
            't': np.array(
              list(map(float, elems[5].split())), np.float_).reshape((3, 1)),
            'time': float(elems[6])
          }
